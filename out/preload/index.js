			...
			const electronAPI = {
				ipcRenderer: {
					...
					sendTo(webContentsId, channel, ...args) {
						...
						const electronMajorVer = electronVer
							? parseInt(electronVer.split(".")[0])
							: 0;
						if (electronMajorVer >= 28) {
							throw new Error(
								'"sendTo" method has been removed since Electron 28.',
							);
						} else {
							electron__WEBPACK_IMPORTED_MODULE_0__.ipcRenderer.sendTo(
								webContentsId,
								channel,
								...args,
							);
					...
					sendSync(channel, ...args) {
						return electron__WEBPACK_IMPORTED_MODULE_0__.ipcRenderer.sendSync(
							channel,
							...args,
						);
					},
					sendToHost(channel, ...args) {
						electron__WEBPACK_IMPORTED_MODULE_0__.ipcRenderer.sendToHost(
							channel,
							...args,
						);
					},
					postMessage(channel, message, transfer) {
						electron__WEBPACK_IMPORTED_MODULE_0__.ipcRenderer.postMessage(
							channel,
							message,
							transfer,
						);
					...
					invoke(channel, ...args) {
						return electron__WEBPACK_IMPORTED_MODULE_0__.ipcRenderer.invoke(
							channel,
							...args,
						);
					},
					...
					removeAllListeners(channel) {
						electron__WEBPACK_IMPORTED_MODULE_0__.ipcRenderer.removeAllListeners(
							channel,
						);
					},
				},
				webFrame: {
					insertCSS(css) {
						return electron__WEBPACK_IMPORTED_MODULE_0__.webFrame.insertCSS(
							css,
						);
					},
					setZoomFactor(factor) {
						if (typeof factor === "number" && factor > 0) {
							electron__WEBPACK_IMPORTED_MODULE_0__.webFrame.setZoomFactor(
								factor,
							);
						}
					},
					setZoomLevel(level) {
						if (typeof level === "number") {
							electron__WEBPACK_IMPORTED_MODULE_0__.webFrame.setZoomLevel(
								level,
							);
						}
					},
				...
				webUtils: {
					getPathForFile(file) {
						return electron__WEBPACK_IMPORTED_MODULE_0__.webUtils.getPathForFile(
							file,
						);
					},
				},
