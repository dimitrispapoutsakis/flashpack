		...
		"./node_modules/@electron-toolkit/utils/dist/index.mjs": (
			...
			const electronApp = {
				setAppUserModelId(id) {
					if (platform.isWindows)
						electron__WEBPACK_IMPORTED_MODULE_0__.app.setAppUserModelId(
							is.dev ? process.execPath : id,
						);
				},
				setAutoLaunch(auto) {
					if (platform.isLinux) return false;
					const isOpenAtLogin = () => {
						return electron__WEBPACK_IMPORTED_MODULE_0__.app.getLoginItemSettings()
							.openAtLogin;
					};
				...
				skipProxy() {
					return electron__WEBPACK_IMPORTED_MODULE_0__.session.defaultSession.setProxy(
						{ mode: "direct" },
					);
				},
			};
			...
			const ipcHelper = {
				handle(channel, listener) {
					...
					electron__WEBPACK_IMPORTED_MODULE_0__.ipcMain.handle(
						channel,
						listener,
					);
				},
				...
				removeAllListeners() {
					listeners.forEach((c) =>
						electron__WEBPACK_IMPORTED_MODULE_0__.ipcMain.removeAllListeners(c),
					);
					listeners = [];
					return this;
				},
				removeAllHandlers() {
					handlers.forEach((c) =>
						electron__WEBPACK_IMPORTED_MODULE_0__.ipcMain.removeHandler(c),
					);
					handlers = [];
				},
			...
			const optimizer = {
				watchWindowShortcuts(window, shortcutOptions) {
					...
					const { escToCloseWindow = false, zoom = false } =
						shortcutOptions || {};
				...
				registerFramelessWindowIpc() {
					electron__WEBPACK_IMPORTED_MODULE_0__.ipcMain.on(
						"win:invoke",
						(event, action) => {
							const win =
								electron__WEBPACK_IMPORTED_MODULE_0__.BrowserWindow.fromWebContents(
									event.sender,
								);
							if (win) {
								if (action === "show") {
